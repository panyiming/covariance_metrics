...
ew_portfolio <- function(df_rolls, N){
  month_days = 21
  res_all = c()
  for(i in c(1:190)){
    st_sam  = (i-1) * 1281 + 1
    ed_sam = st_sam + 1260 - 1
    st_pre = ed_sam + 1
    ed_pre = st_pre + 21 - 1
...
samcov_portfolio <- function(df_rolls, N){
  month_days = 21
  res_all = c()
  for(i in c(1:190)){
    st_sam  = (i-1) * 1281 + 1
...
all_res_ew = ew_portfolio(df_rolls, 500)
all_sam_ew = samcov_portfolio(df_rolls, 500)
```
...
library(rugarch)
library(xdcclarge)
test   = read.csv('./test_500_1260.csv')
...
fitlist = multifit(multispec=mspec, 
        data = Rtn) 
ht<-sigma(fitlist)^2 
